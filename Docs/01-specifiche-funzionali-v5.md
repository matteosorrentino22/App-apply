...
> **Versione:** 5 — integra le decisioni del documento 03 (contenuto del CV) e della chat di recepimento (luglio 2026)
> **Stato:** revisionato
...
>
> **Nota di versione (v5).** Questa revisione recepisce il documento `03-specifiche-funzionali-contenuto-cv-v3.md` (che in caso di conflitto su questi temi prevale) e le decisioni della chat di recepimento:
> 1. **Sezione "risultati chiave" eliminata dal profilo master.** Non è mai stata un campo compilato in modo affidabile e generava confusione col contenuto CV. Il profilo master ha ora **cinque** sezioni (non sei): sommario professionale, esperienze, istruzione, competenze e certificazioni, lingue. *(Vedi §4.1, §7.)*
> 2. **"Areas of Expertise" è contenuto del CV, non del profilo.** È calcolato dal modello a ogni generazione, per sintesi/astrazione da esperienze e bullet reali — non un campo che l'utente compila. *(Vedi doc 03 §5.3.)*
> 3. **Sezione istruzione: "invariata" precisato in "non riformulata".** Il testo non viene mai riscritto dall'AI, ma è ora soggetta a **selezione** (max 3 voci più recenti, tie-break) e a vincoli di input (data di fine obbligatoria). *(Vedi §4.1, §4.7.)*
> 4. **Arricchimento ristretto e con priorità.** L'arricchimento può aggiungere solo attività/progetti agganciati a un'esperienza **già presente** nel profilo, non un'esperienza intera nuova; ha priorità massima di inclusione nel CV per cui è inserito. *(Vedi §4.8.)*
> 5. **Nuovo elemento del CV: qualifica professionale.** Riga generata sotto il nome, calcolata dal modello a ogni generazione dal titolo del job (o dall'esperienza più recente in caso di titolo ambiguo; dal titolo del job "pulito" se il profilo non ha esperienze). *(Vedi doc 03 §5.1; nuovo caso limite chiuso in questa revisione.)*
> 6. **Vincoli minimi del profilo master.** Almeno **1 voce di istruzione obbligatoria** (con data di fine obbligatoria); le **esperienze possono essere 0** (caso neolaureato). *(Vedi §4.1, §7.)*
> 7. **Nessuna migrazione dati per i nuovi vincoli.** L'app è ancora in fase di test senza profili da preservare: i nuovi vincoli si applicano da subito, senza logica di retrocompatibilità.
...
- Profilo master strutturato in sezioni: sommario professionale, esperienze lavorative, istruzione, competenze e certificazioni, lingue. *(La sezione "risultati chiave" non esiste più nel profilo — v5: "Areas of Expertise" è contenuto sintetizzato dal modello nel CV, non un campo compilato dall'utente — vedi doc 03 §5.3.)*
- **Vincoli minimi di completezza del profilo:** almeno **1 voce di istruzione** è obbligatoria (con **data di fine obbligatoria** per ogni voce); le **esperienze lavorative possono essere 0** (caso legittimo, es. neolaureato). *(Vedi doc 03 §3.1–3.2.)*
...
**Contenuto del CV.** Il CV combina profilo master + descrizione della posizione: riformula le sezioni pertinenti per far emergere l'esperienza reale più rilevante, **non riformula mai la sezione istruzione** (il testo resta quello del profilo, ma è soggetto a selezione delle voci più recenti — vedi doc 03 §3.2), e **non aggiunge nulla che non sia nel profilo** (salvo i dettagli aggiunti dall'utente via arricchimento — §4.8). Le regole dettagliate su quantità, selezione e struttura del contenuto (esperienze mostrate, budget bullet, Areas of Expertise, qualifica professionale, ecc.) sono definite nel documento `03-specifiche-funzionali-contenuto-cv-v3.md`.
...
> **Nota (v5).** Regole aggiornate secondo doc 03 §5.6, che emenda quanto segue.

- Prima di generare **o rigenerare** un CV, l'utente può aggiungere un dettaglio (nuove attività/progetti) **agganciato a un'esperienza già presente nel profilo master**, per colmare una lacuna evidenziata. **Non può introdurre un'esperienza (azienda/ruolo) del tutto assente dal profilo.**
...
- **Priorità di inclusione:** il contenuto di arricchimento inserito per il CV corrente ha priorità massima — è tra gli ultimi a essere rimosso in caso di overflow (doc 03 §6). Se l'utente lo salva anche nel profilo master, nei CV **successivi** è trattato come contenuto normale, senza priorità.
- L'arricchimento tocca **solo il CV generato**: non ricalcola il punteggio e non sblocca la generazione automatica per job sotto soglia. Di conseguenza, un CV può contenere attività reale non (ancora) presente nel master.
...
- ☐ Il profilo master presenta tutte le sezioni previste (sommario, esperienze, istruzione, competenze/certificazioni, lingue) — nessuna sezione "risultati chiave".
- ☐ Il profilo è considerato completo con almeno 1 voce di istruzione (data di fine obbligatoria) e 0 o più esperienze.
...
- ☐ Prima di generare o rigenerare un CV, l'utente può aggiungere un dettaglio agganciato a un'esperienza già presente nel profilo.
- ☐ Non è possibile, via arricchimento, introdurre un'esperienza (azienda/ruolo) del tutto assente dal profilo.
...
- ☐ Il contenuto di arricchimento ha priorità massima di inclusione nel CV per cui è inserito.
- ☐ Se salvato anche nel profilo master, nei CV successivi è trattato come contenuto normale (senza priorità).
...
Dubbi residui e decisioni consapevoli da riesaminare nelle fasi successive. Rispetto alla v4 sono stati **chiusi** dal documento 03 e dalla chat di recepimento: la struttura e le quantità del contenuto del CV (esperienze mostrate, budget bullet, istruzione mostrata), il naming "risultati chiave"/"Areas of Expertise" (sezione eliminata dal profilo, contenuto ora solo generato nel CV), l'ambito dell'arricchimento (ristretto a esperienze esistenti), il comportamento in caso di overflow estremo del CV, e la retroattività dei nuovi vincoli di profilo (non applicabile: nessun dato da migrare). La numerazione è stata rifatta.
...
22. **Scelta casuale nel tie-break.** Con data mancante o pareggio persistente al taglio dei 15, la scelta è casuale: due esecuzioni sugli stessi dati possono selezionare job diversi. Accettato (il caso è marginale); segnalato perché rende il taglio non perfettamente riproducibile.

23. **Calibrazione numerica del contenuto del CV** *(nuovo, v5)*. I parametri del documento 03 (budget bullet per voci di istruzione, formula del peso recenza+durata, `EDU_MAX_SHOWN`, minimo bullet garantito, limiti di caratteri) sono valori indicativi da tarare in fase di test/tuning — vedi doc 03 §11.1.

24. **Esaurimento del loop di ripiego per overflow** *(nuovo, v5)*. Confermato comportamento silenzioso (PDF multi-pagina salvato senza errore, con log backend) per il caso — atteso irraggiungibile — in cui anche dopo il taglio massimo il contenuto ecceda 1 pagina.
