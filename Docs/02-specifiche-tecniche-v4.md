...
> **Versione:** 4 — integra il modello dati e la pipeline di generazione CV del documento 03 (luglio 2026)
> **Stato:** revisionato
...
>
> **Nota di versione (v4).** Questa revisione recepisce `03-specifiche-funzionali-contenuto-cv-v3.md`:
> 1. **`Profile.key_achievements` rimosso.** Non esiste più una sezione "risultati chiave" nel profilo. "Areas of Expertise" è contenuto calcolato dal modello a ogni generazione, non un campo persistito nel profilo (§4.2).
> 2. **`Education` ristrutturato.** Il campo libero `dates` è sostituito da `start_date` (opzionale) e `end_date` (**obbligatorio**), necessario per il tie-break di selezione delle 3 voci più recenti (§4.2).
> 3. **Pipeline di generazione CV arricchita.** Nuovo step per la qualifica professionale, calcolo deterministico del budget bullet, loop di ripiego per overflow, riferimento di grounding per le Areas of Expertise (§6).
> 4. **`CVDocument` / `Job`**: nessuna modifica di schema oltre a quanto già presente; il contenuto aggiuntivo (qualifica, ordinamento di rilevanza restituito dal modello) è parte di `html_source`/dati intermedi di generazione, non richiede nuove tabelle.
> 5. **Nessuna migrazione dati.** L'app è in fase di test senza profili da preservare: i nuovi vincoli (es. `end_date` obbligatorio) si applicano da subito.
...
Il profilo master è strutturato in sezioni (§4.1 funzionale, v5). Si modella come un profilo principale con tabelle collegate per gli elementi ripetibili:
- `Profile` — `user`, `summary` (sommario professionale), `photo` (immagine). **Nessun campo `key_achievements`** (rimosso in v4: "Areas of Expertise" è contenuto calcolato dal modello a ogni generazione del CV, non un dato persistito nel profilo — vedi doc 03 §5.3 e §6.2 di questo documento).
- `Experience` — `profile`, `company`, `role`, `location`, `start_date`, `end_date`, `bullets` (elenco di punti, ciascuno con limite di lunghezza — parametro configurabile, indicativo 200 caratteri), `technologies` — **una riga per ogni esperienza** (numero variabile: è ciò che rende il CV dinamico). Numero massimo di bullet per esperienza: parametro configurabile (indicativo: 8). Nessun limite al numero di esperienze inseribili (0 o più ammesse); superate le 5, l'app avvisa che il CV ne mostrerà al massimo 5 (doc 03 §3.1).
- `Education` — `profile`, `institution`, `title`, `location`, `start_date` (opzionale), **`end_date` (obbligatorio, non nullable)**, `notes` — **sezione mai riscritta dall'AI**, ma soggetta a selezione delle voci più recenti (doc 03 §3.2). Almeno **1 voce è richiesta** per considerare il profilo completo (validato in onboarding). Limiti di lunghezza per campo: parametri da definire (doc 03 §11.1).
- `Skill` / `Certification` — voci di competenze e certificazioni (hard skill — confluiscono in "Skilled in"/"Certified in" del CV, doc 03 §5.5)
...
**Nota di validazione (onboarding/profilo).** Il profilo è considerato "completo" (generabile) solo se: almeno 1 `Education` con `end_date` valorizzata; 0 o più `Experience`. Nessun vincolo di retrocompatibilità: essendo l'app ancora in fase di test, questi vincoli si applicano da subito senza dati legacy da migrare.
...
> **Aggiornata in v4** secondo `03-specifiche-funzionali-contenuto-cv-v3.md`: le quantità (esperienze mostrate, budget bullet, voci istruzione) sono calcolate **deterministicamente dal sistema**, non lasciate al modello. Il modello decide i contenuti entro quelle quantità e restituisce un ordinamento di rilevanza usato per il taglio in caso di overflow.

1. **Raccolta contesto.** Si prende il profilo master strutturato dell'utente (§4.2) + la descrizione del job + (opzionale) il dettaglio di arricchimento (§4.8 funzionale, ristretto a esperienze esistenti — doc 03 §5.6).
2. **Calcolo deterministico delle quantità (lato server, prima della chiamata AI).**
   - Voci di istruzione da mostrare: le **3 più recenti** (`EDU_MAX_SHOWN`, parametro), con tie-break su durata minore e infine casuale (doc 03 §3.2).
   - Esperienze da mostrare: se ≤5, tutte; se >5, le 5 più recenti (soggette a swap con esperienze escluse marcate "altamente rilevanti" dal modello — vedi punto 3).
   - **Budget bullet totale (B)**, in funzione del numero di voci di istruzione mostrate (tabella parametrica, es. 1 voce → B=12, 2 → B=10, 3 → B=9), poi distribuito tra le esperienze per peso (recenza + durata, formula parametrica — valori da calibrare, doc 03 §11.1).
3. **Chiamata a Claude per i contenuti.** Claude riceve i dati reali dell'utente, il job, e le quantità calcolate al punto 2. Produce: sommario riformulato, **qualifica professionale** (nuovo campo — nome del ruolo generalizzato dal titolo del job, o dall'esperienza più recente se il titolo è ambiguo, o dal titolo "pulito" se non esistono esperienze — doc 03 §5.1), bullet delle esperienze selezionati/riscritti entro il budget assegnato, **Areas of Expertise** (4–6 voci, sintesi per astrazione da esperienze/bullet reali, mai dedotte dalla sola job description, ciascuna con un riferimento interno di grounding all'esperienza/bullet di provenienza — doc 03 §5.3), selezione di Skills/Certificazioni (hard skill, letterali dal profilo — doc 03 §5.5). Il modello restituisce inoltre, per ogni esperienza/bullet prodotto, un **ordinamento di rilevanza** rispetto al job (usato al punto 5) e marca le esperienze escluse come "altamente rilevanti" se pertinente (per gli swap del punto 2). Regole di grounding invariate: non inventare nulla fuori dal profilo (eccetto la sintesi per astrazione delle Areas of Expertise), tradurre sì, rispettare la lunghezza.
4. **Composizione HTML.** I contenuti si inseriscono in un **template HTML flessibile** (uno solo nell'MVP, §6.4): un ciclo ripete il blocco "esperienza"; la sezione **istruzione** è copiata senza riformulazione dal profilo, limitata alle voci selezionate al punto 2; la foto è inclusa **solo** se l'utente ha attivato l'opzione (§6.5); le etichette di sezione ("Areas of Expertise", "Certified in", "Skilled in") seguono `cv_language_mode`.
5. **Controllo "1 pagina" e loop di ripiego.** Il template HTML/CSS è progettato per stare in una pagina; WeasyPrint verifica il numero di pagine risultanti. In caso di overflow: (a) 3 livelli di compattamento progressivo (font/margini); (b) se ancora in overflow, **loop di ripiego**: rimozione meccanica lato server del bullet globalmente meno rilevante secondo l'ordinamento del punto 3, senza nuova chiamata AI, finché il CV entra in 1 pagina (i bullet di arricchimento sono protetti, rimossi per ultimi); (c) caso residuale (atteso irraggiungibile): se anche a zero bullet il contenuto eccede, si salva comunque il PDF multi-pagina senza errore, con evento tracciato in `RunLog` per diagnosi — nessun messaggio d'errore all'utente (doc 03 §6, §11.3).
6. **PDF.** WeasyPrint converte l'HTML in **PDF** — unico formato di output.
7. **Salvataggio.** HTML e PDF finiscono in `CVDocument`; il job passa a `cv_generated`.
...
Dubbi tecnici residui e decisioni da riesaminare in fase di implementazione o nella successiva revisione di semplicità/manutenibilità. In **v4** sono stati **chiusi**: la struttura del modello dati per Education (data fine obbligatoria), la rimozione di `key_achievements`, il comportamento dell'overflow estremo (silenzioso, PDF multi-pagina + log), la retroattività dei nuovi vincoli (non applicabile, nessun dato legacy). Restano aperti, dal doc 03, i valori numerici di calibrazione (§11.1 doc 03) e i punti elencati sotto. La numerazione è stata rifatta.
...
17. **Calibrazione dei parametri della pipeline di contenuto CV** *(nuovo, v4)*. Budget bullet per numero di voci istruzione (12/10/9), formula peso recenza+durata, minimo bullet garantito a un'esperienza vecchia ma rilevante, limiti di caratteri istruzione: tutti valori indicativi da tarare in test (doc 03 §11.1). Nessun impatto architetturale: sono parametri di configurazione applicativa, come i prezzi del credito (§8.5).

18. **Grounding verificabile delle Areas of Expertise.** Il riferimento interno di provenienza (bullet/esperienza) che il modello restituisce per ogni area (doc 03 §5.3) va salvato come dato intermedio della generazione (non richiede nuova tabella: può stare nei dati di lavorazione del task Celery o in un campo JSON di servizio su `CVDocument`), utile per diagnosi in fase di tuning del prompt.
